...
//--- 키값이 key인 데이터를 data의 요소로 추가 ---//
	public int add(SimpleObject5 st, Comparator<? super SimpleObject5> c) {
		System.out.println("st.no = " + st.no); // 사실상 no이 키값이다.
		int hash = Integer.parseInt(st.no) % 13; // 추가할 데이터의 해시값 = 인덱스라고 생각하면 된다.
		System.out.println("hash 값 = " + hash);
		Node2 p = table[hash]; // 선택 노드

		while (p != null) {
			if (c.compare(st, p.data) == 0) // 키값이 이미 등록됨 > 데이터가 들어가지 않는다.
				return 1;
			p = p.next; // 다음 노드를 선택
		}
...
public class 객체체인해시 {
	...
	public static void main(String[] args) {
		...
		SimpleObject5 data; // 넘버와 이름이 저장된 데이터
		int select = 0, result = 0;
		do {
			switch (menu = SelectMenu()) {
			case Add:
				data = new SimpleObject5();
				data.scanData("삽입", SimpleObject5.NO | SimpleObject5.NAME); // 비트연산자
